...
## Active Projects
See [project_foo.md](project_foo.md) for the Foo migration, in progress.
...
**1. Compress index entries to one line.** A pointer doesn't need a paragraph. `See [foo.md](foo.md) for a one-line hook of why future-you will want this.` Anything richer belongs in `foo.md` itself. Aim for ~150 chars per index line; the index is a table of contents, not a summary.
